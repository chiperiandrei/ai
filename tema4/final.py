...
class Table:
    ...
    def minimaxAB(self, state=None, maximizer=True, depth=0, alpha=-infinity, beta=infinity, max_depth=2):
        if state is None:
            state = self.table
        ...
        if maximizer:
            value = -infinity
            for action in self.get_actions(state):
                _, h = self.minimaxAB(action, False, depth + 1, alpha, beta)
                value = max(value, h)
            ...
            value = infinity
            for action in self.get_actions(state):
                _, h = self.minimaxAB(action, True, depth + 1, alpha, beta)
                value = min(value, h)
                beta = min(beta, value)
                if beta <= alpha:
                    break
        ...
        for i in n:
            for j in n:
                if self.table[i][j] is not None:
                    self.table[i][j].position['row'] = i
                    self.table[i][j].position['col'] = j
    ...
    def drawGame(self):
        for i in range(4):
            for j in range(4):
                ...
                for k in range(4):
                    piece_computer = self.get_piece(k + 1, 'Computer')
                    r = piece_computer.position['row']
                    c = piece_computer.position['col']
                    x1 = 50 + c * 100 + (c + 1) * 10
                    y1 = 50 + r * 100 + (r + 1) * 10
                    pygame.draw.circle(
                        self.screen,
